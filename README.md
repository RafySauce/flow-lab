...
- **[`flow-foundry/`](flow-foundry/)** scaffolds flowspaces: stage decomposition, per-stage contracts (Inputs / Process / Outputs / Verify, plus two workplace extensions — Review and Data boundary), a review-intensity map, and validation gates. Read its [README](flow-foundry/README.md) and [foundry-spec](flow-foundry/foundry-spec.md).
- **[`skill-foundry/`](skill-foundry/)** authors skills as an **engine-neutral spec** plus per-engine adapters (Rovo agent definition, Copilot custom agent / prompt file). Read its [README](skill-foundry/README.md) and [foundry-spec](skill-foundry/foundry-spec.md).
