...
What context/sources the skill needs (pages, boards, repos, documents). Max `data-class` it will handle. Which engine(s) it should run on — and note if that's a constraint with its reason (e.g., "must be Rovo: the invoking users work in Atlassian chat," or "this data class isn't sanctioned for Copilot per the employer matrix"). Both engines ground on the source-repo (Rovo via the GitLab connector), so repo access alone is never the constraint.
