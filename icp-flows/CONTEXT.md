...
> **Before you invoke one:** everything above is a sanitized *design*, not a live agent. Each flow assumes an employer-side GitLab instance (the sole source of truth for that flow) that must exist before any stage can actually run — opening a `HUB.md` shows you the design and what instantiation requires, it doesn't run anything by itself. See [`README.md`'s "Using what's already built"](../README.md#using-whats-already-built) for the path from design to running instance.
