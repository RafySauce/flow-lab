...
It holds **method, templates, and sanitized exemplars only**. No employer content, no personal data, no credentials — ever. Instantiated flowspaces live in the operator's employer tenancy (an internal GitLab repository — the *source-repo*, the sole source of truth for an instance), not here.
...
- **A session that opens with only a repo reference and nothing else** (a pasted link, a connected repo, no other stated ask) → treat this the same as "what can this repo do for me?" above: default to `START-HERE.md` first, not a neutral "explain this repo" survey. An unstated intent is not a build instruction — it defaults to use, per the rule above — and a general survey is what leads an agent into `README.md`'s methodology/foundry sections before ever reaching the "use it now" path. This default yields immediately to explicit build language, which still routes to rule 2 below.
...
| Understand the source-repo model / engine handoffs | `methodology/mirroring-protocol.md` |
